...
API_KEY = 'db89b5c66fabea466443ab0e9d81dfdb'
...
def logIn(driver, mail, password, window):
    try:
        ...
        try:
            skip_button_vk = driver.find_element(By.CSS_SELECTOR, '#root > div > div > div > div > div > form > div:nth-child(2) > div > div.iGVaLv1nniQGGeWuC1H_ > button:nth-child(2)')
            skip_button_vk.click()
        except:
            pass

        driver.implicitly_wait(5)
        try:
            iframe = driver.find_element(By.ID, "pass")
            driver.switch_to.frame(iframe)
            password_input = driver.find_element(By.CSS_SELECTOR, '#password').send_keys(password)
            driver.switch_to.default_content()
            time.sleep(2)
            button = (driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/section/form/div[2]/button'))
            button.click()
        except Exception as e:
            print(e)
            password_input = driver.find_element(By.XPATH, '//input[@name="password"]').send_keys(password)
